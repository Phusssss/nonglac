...
const CreatePostForm = ({ onPostCreated, setShowLoginModal }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!newPost.title.trim() || !newPost.content.trim()) {
      messageApi.error('Vui lòng nhập đầy đủ tiêu đề và nội dung');
      return;
    }
    ...
    try {
      ...
      await logUserAction(
        user.uid,
        userProfile?.displayName || user.displayName || user.email || 'Người dùng',
        ACTIONS.CREATE_POST,
        { category: postData.category }
      );
      messageApi.success('Đã đăng bài viết thành công!');
      ...
      messageApi.error('Có lỗi xảy ra khi đăng bài viết');
    } finally {
      setSubmitting(false);
    }
  };
